...
static LogicalResult checkExtensionRequirements(
    ...
    const ArrayRef<spirv::Extension> &skippedCandidates = {}) {
  for (const auto &ors : candidates) {
    if (targetEnv.allows(ors) ||
        llvm::any_of(skippedCandidates, [&](spirv::Extension skippedExt) {
          return llvm::is_contained(ors, skippedExt);
        }))
      continue;
...
static LogicalResult checkCapabilityRequirements(
    ...
    const ArrayRef<spirv::Capability> &skippedCandidates = {}) {
  for (const auto &ors : candidates) {
    if (targetEnv.allows(ors) ||
        llvm::any_of(skippedCandidates, [&](spirv::Capability skippedCap) {
          return llvm::is_contained(ors, skippedCap);
        }))
      continue;
...
static LogicalResult checkCapabilityAndExtensionRequirements(
    ...
    const ArrayRef<spirv::Capability> &skippedCapCandidates = {},
    const ArrayRef<spirv::Extension> &skippedExtCandidates = {}) {
...
convertVectorType(const spirv::TargetEnv &targetEnv,
  ...
  llvm::SmallVector<spirv::Capability, 4> skippedCaps;
  llvm::SmallVector<spirv::Extension, 4> skippedExts;

  // Relax the bitwidth requirements for capabilities and extensions
  if (options.emulateLT32BitScalarTypes) {
    skippedCaps.push_back(spirv::Capability::Int8);
  ...
  // that were relaxed (e.g., skippedCaps)
  for (auto cap : skippedCaps) {
    llvm::Optional<::llvm::ArrayRef<::mlir::spirv::Extension>> ext =
        mlir::spirv::getExtensions(cap);
    if (ext.has_value())
      skippedExts.insert(skippedExts.end(), ext.value().begin(),
                         ext.value().end());
  }
  // If all requirements are met, then we can accept this type as-is.
  if (succeeded(checkCapabilityAndExtensionRequirements(
          type, targetEnv, capabilities, extensions, skippedCaps, skippedExts)))
    return type;
  else {
    return nullptr;
  }

...
void buildGPUPassPipeline(OpPassManager &pm,
  ...
  if (options.xegpuOpLevel == "workgroup") {
    ...
    // Experimental: Convert chained multi_reduction to tree reduction
    pm.addNestedPass<gpu::GPUModuleOp>(
        vector::createVectorMultiReductionToTreeReductionPass());
    pm.addNestedPass<gpu::GPUModuleOp>(createCSEPass());
  }
  ...
  // Optimize shuffle-based reductions to SPIRV group intrinsics
  // pm.addNestedPass<gpu::GPUModuleOp>(createOptimizeShuffleReductionsPass());
  ...
  // around a supported type (defaulting to f32).
  {
    ...
    mathExtendOptions.extraTypeStrs.assign(

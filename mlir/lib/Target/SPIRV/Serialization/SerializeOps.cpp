...
LogicalResult Serializer::processFuncOp(spirv::FuncOp op) {
  ...
  // @mshahneo:
  // We handle external functions with LinkageAttributes differently
  if (op.isExternal()) {
    if(op->getAttr("LinkageAttributes")) {
      ...
      // @mshahneo: using the block generated directly, instead of getting it from 
      // the definition
      // ==========================================
      ...
      // Now remove the body, since it's use is done
      op.eraseBody();
